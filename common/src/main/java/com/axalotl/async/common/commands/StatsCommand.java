...
public class StatsCommand {
    ...
    private static void showGeneralStats(CommandSourceStack source) {
        int activeThreads = ParallelProcessor.getActiveThreadCount();
        int poolSize = ParallelProcessor.getPoolSize();
        long queuedTasks = ParallelProcessor.getQueuedTaskCount();

        MutableComponent message = prefix.copy()
                .append(Component.literal("Performance Statistics ")
                        .withStyle(style -> style.withColor(ChatFormatting.GOLD)))
                .append(Component.literal("\nThread Pool: ").withStyle(style -> style.withColor(ChatFormatting.WHITE)))
                .append(Component.literal(String.valueOf(activeThreads))
                        .withStyle(style -> style.withColor(ChatFormatting.GREEN)))
                .append(Component.literal("/" + poolSize + " active")
                        .withStyle(style -> style.withColor(ChatFormatting.GRAY)))
                .append(Component.literal("\nQueued Tasks: ").withStyle(style -> style.withColor(ChatFormatting.WHITE)))
                .append(Component.literal(String.valueOf(queuedTasks))
                        .withStyle(style -> style.withColor(ChatFormatting.AQUA)))
                .append(Component.literal("\nAsync Status: ").withStyle(style -> style.withColor(ChatFormatting.WHITE)))
                .append(Component.literal(AsyncConfig.disabled.getValue() ? "Disabled" : "Enabled")
                        .withStyle(style -> style.withColor(
                                AsyncConfig.disabled.getValue() ? ChatFormatting.RED : ChatFormatting.GREEN)));
        ...
        server.execute(() -> {
            ...
            if (topCount > 0) {
                ...
                if (!sortedEntities.isEmpty()) {
                    message.append(Component.literal("\n\nTop " + sortedEntities.size() + " Entity Types:")
                            .withStyle(style -> style.withColor(ChatFormatting.GOLD)));
    ...
    private static void updateStats() {
        ...
        // Use real ForkJoinPool active thread count
        int currentThreads = ParallelProcessor.getActiveThreadCount();
